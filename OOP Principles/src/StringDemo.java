...
public class StringDemo {
	...
	public static void main(String[] args) {
		/*
		 * When the new keyword is used, new memory will always be created. However,
		 * we do not need to use this keyword with String objects. Since they are
		 * immutable, Java will reuse String objects when possible if the new
		 * keyword is not used.
		 */
		...
		/*
		 * Since the equals() method uses the hashCode(), all of these Strings are
		 * considered equivalent to each other. We actually want this behavior---if
		 * the String objects have the same value, they should be considered
		 * equivalent.
		 */
		...
		/*
		 * But, the hash code here is misleading. Any class can override this method
		 * to return something OTHER than the identity hashcode (e.g. memory
		 * location/unique object identifier). To view the actual identity hashcode,
		 * you must use the identityHashCode() method whenever dealing with a class
		 * other than Object.
		 */
		 ...
		 * class overrode equals() to compare by content. But, the == operator still
		 * compares by identity hashcode!
		 */
		 ...
		 * equals() to compare by value, not the identity hashcode. But the ==
		 * operator will *always* compare by identity hashcode.
		 */
	}
}
