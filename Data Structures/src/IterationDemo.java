...
public class IterationDemo {
	...
	/*
	 * Example 1: Uses a traditional for loop in an attempt to output the map (but
	...
	public void forExample() {
		 ...
		 * loop, but this approach does not produce the correct output. It
		 * incorrectly assumes that the last key value is less than the size of the
		 * map. In the demo, the last key value is 12, but the size of the map
		 * (determined by the number of keys) is 5.
		 *
		 * Output Inefficient: It took 5 outer iterations to output 2 results from
		 * our map. We can do much better than that!
		 */
	}
	...
	public void whileExample() {
		 ...
		 * correct output. Instead of looping until a key larger than the size is
		 * reached, it loops until results.size() keys have been found.
		 *
		 * Output Inefficient: This is unnecessarily inefficient. What happens if
		 * there are only two keys... 2 and 100. We loop through 100 times to get
	...
	public void foreachExample() {
		 ...
		 * iterator, but results in easier to read code. It also removes the threat
		 * of off-by-one errors that comes with traditional for loops.
		 *
	...
	public void mapentryExample() {
		...
		// With Java 10 we can just say "var entry" to avoid the ugly type
	...
	public void treemapExample() {
		 ...
		 * lowerKey() methods to explicitly iterate through the map in sorted order.
		 * We can even use these to iterate through the keys in reverse.
		 *
